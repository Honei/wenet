...
data=/home/users/xiongxinlei/dataset/aishell/
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  # 计算cmvn
  # nj=$(get_nj data/${train_set}/wav.scp)
  # 使用训练集中提取全局的CMVN数据
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  # 然后去重排序
  # 去掉全部是空格的那些行
  # 然后将每个词进行编号，序号从2开始，0和1已经被占用了
  # 在aishell数据集中，目前只有4232个单元，汉字4229，3个特殊符号<blank><unk>和<sos/eos>
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  # 7.1 Prepare dict
  # 准备词典，目前是字粒度建模，因此要将所有的词转换为由字组成的形式
  echo "prepare the language model dict"
  unit_file=$dict
